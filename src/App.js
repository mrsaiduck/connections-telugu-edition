  ...
  return (
    <main className={`absolute flex flex-col justify-center items-center w-full h-full z-10 bg-purple-300 border-black border-y gap-16 transition-opacity duration-300 ${state ? "opacity-0 pointer-events-none" : "opacity-100 pointer-events-auto"}`}> 
      ...
      <div className='flex flex-col justify-center items-center w-full gap-8'>
        ...
        <h1 className="text-5xl font-extrabold">కనెక్షన్లు</h1>
        <p>Group words that share a common thread [Telugu Edition].</p>  
      ...
      <div className='flex flex-col justify-center items-center w-full gap-2'>
        ...
        <h1 className='text-sm'>By Yashwanth Sai Muddam [ABCD]</h1>
      </div>
...
const Game = ({ state }) => {
  ...
  function getSolutionWords() {
    const allSolutions = Object.entries(solutions); // Convert the object into an array of [key, value] pairs
    const selectedSolutions = shuffle(allSolutions).slice(0, 4); // Shuffle and select 4 groups
  
    const solutionWords = [];
  
    // Loop through each selected solution and get difficulty + words
    for (const [key, solution] of selectedSolutions) {
  ...
  const WordButton = ({ id, word, selected }) => {
    return (
      <button 
        className={`w-full h-18 ${selected ? selectedColor : unselectedColor} rounded-lg drop-shadow-sm lg:h-20 max-h-[100%]`}
        onClick={() => handleWordButtonClick(id)}
      >
        <h1 className='text-black font-bold text-xxxs uppercase select-none lg:text-s break-all' style={{ whiteSpace: 'pre-wrap', overflow: 'hidden' }}>{word}</h1>
      </button>
    )
  }
  ...
  return (
    <main className={`absolute flex flex-col justify-start items-center w-full h-full bg-white border-black border-y gap-10 p-2 transition-opacity duration-300 ${state ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}> 
      ...
      <h1 className='text-xl select-none'>Create groups of four!</h1>
      ...
      <div className='flex flex-col justify-items-center items-center w-full h-1/2 gap-10' style={{ marginBottom: "2rem", marginTop: "1rem" }}>
    {
        winStatus === "" ? (
            <div className='flex flex-col justify-center items-center w-1/2 gap-3 select-none'>
                <h1 className="mb-2">Remaining errors:</h1>
    ...
    <div className='flex flex-row justify-center items-center w-1/2 gap-3'>
        {
            winStatus === "" ? (
                <>
                    ...
                    >
                        <h1 className='text-xs font-medium select-none lg:text-md'>Shuffle</h1>
                    </button>
                    ...
                    >
                        <h1 className='text-xs font-medium select-none lg:text-md'>Deselect All</h1>
                    </button>
                    ...
                    >
                        <h1 className='text-xs font-medium select-none lg:text-md'>Submit</h1>
                    </button>
                </>
            ) : (
                <button
                ...
                >
                    <h1 className='text-xs font-medium select-none lg:text-md'>Results</h1>
                </button>
            )
        }
    </div>
